...
data_df        <- data_df %>% filter(!is.na(atAll)) #excluding NA
...
#保留解释变异最大的2个随机效应
m2.at.re1 <- lmer(atAll ~ 1  + type * consistency2 +
                        (1 | subid), 
                      data = data_df,
                      control = lmerControl(optimizer = 'bobyqa',
                                            calc.derivs = F,
...
summary(m2.at.re1)
isSingular(m2.at.re1)
summary(rePCA(m2.at.re1))
save(m2.at.re1, file = "m2_1res_atAll.RData")
...
#consistency主效应
emm_at <- emmeans(m2.at.re1, ~ consistency2)
emm_at

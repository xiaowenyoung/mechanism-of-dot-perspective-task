...
data_df        <- data_df %>% filter(!is.na(againstAll)) #excluding NA
...
#保留解释变异最大的2个随机效应
m2.against.re1 <- lmer(againstAll ~ 1  + type * consistency2 +
                        (1 | subid), 
                      data = data_df,
                      control = lmerControl(optimizer = 'bobyqa',
                                            calc.derivs = F,
...
summary(m2.against.re1)
isSingular(m2.against.re1)
summary(rePCA(m2.against.re1))
save(m2.against.re1, file = "m2_1res_againstAll.RData")
...
anova(m2.against.re1, m2.against.full, refit = F)

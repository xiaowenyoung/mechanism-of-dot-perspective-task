...
#首先对全模型进行拟合
m3.PorF.full <- glmer(PorFNum ~ 1 + type * consistency2 + 
                        (1 + type * consistency2 | subid), 
                      data = data_df,
                      family="poisson")
                      #此处注视次数为不连续变量，分布为泊松分布

summary(m3.PorF.full)
isSingular(m3.PorF.full) #查看是否奇异拟合

```
```{r rePCA1}

#使用PCA对模型内的参数进行分析
summary(rePCA(m3.PorF.full)) # to see how many random effects are needed. It seems 3 is good enough, not the full 6

model_terms <- model.matrix(m3.PorF.full)  ### Please, please check the model_terms very very carefully. #see also:  https://rpubs.com/Reinhold/22193 and https://rpubs.com/yjunechoe/correlationsLMEM

p_name   <- colnames(model.matrix(m3.PorF.full)) 
...
re_t          <- model.matrix(m3.PorF.full)[,2] # random effects of type
re_c          <- model.matrix(m3.PorF.full)[,3] # random effects of consisc
re_i1         <- model.matrix(m3.PorF.full)[,4] # random effects of consisi1
re_tc         <- model.matrix(m3.PorF.full)[,5] # random effects of type:c
re_ti1        <- model.matrix(m3.PorF.full)[,6] # random effects of type:i1

#This should be the same as Full model
m3.PorF.full2 <- glmer(PorFNum ~ 1  + type * consistency2 +
                        (1 + re_t + re_c + re_i1 +re_tc +re_ti1 | subid), 
                      data = data_df,
                      family="poisson")

summary(m3.PorF.full2)
#注意和第一个全模型进行仔细比对
...
#保留解释变异最大的2个随机效应
m3.PorF.re2 <- glmer(PorFNum ~ 1  + type * consistency2 +
                        (1 + re_c | subid), 
                      data = data_df,
                      family="poisson")

summary(m3.PorF.re2)
isSingular(m3.PorF.re2)
summary(rePCA(m3.PorF.re2))
save(m3.PorF.re2, file = "m3_2res_PorFNum.RData")
...
anova(m3.PorF.re2, m3.PorF.full, refit = F)
...
PorFNum.sig2 <- mixed(PorFNum ~ 1  + type * consistency2 +
                        (1  + re_c | subid), 
                      data = data_df,
                      family="poisson",
                      method='LRT',
...
PorFNum.sig2
save(PorFNum.sig2, file = "PorFNum_sig.RData")
...
emmip(m3.PorF.re2, ~ consistency2) #simple plotting

#consistency主效应
emm_head <- emmeans(m3.PorF.re2, ~ consistency2)
emm_head

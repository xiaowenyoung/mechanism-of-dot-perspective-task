...
data_df$headAll[is.na(data_df$headAll)]<- 0 #将N因变量下的NA换成0
...
model_terms <- model.matrix(m2.head.full)
...
#PCA suggests that 3 random effects are sufficient to interpret all the variance
m2.head.re3 <- lmer(headAll ~ 1  + type * consistency2 +
                        (1 + re_ti1 + re_i1 | subid), 
                      data = data_df,
                      control = lmerControl(optimizer = 'bobyqa',
                                            calc.derivs = F,
                                            optCtrl = list(maxfun = 2e5)))
...
summary(m2.head.re3)
isSingular(m2.head.re3)
summary(rePCA(m2.head.re3))
save(m2.head.re1, file = "m2_3res_headAll.RData")
...
anova(m2.head.re3, m2.head.full, refit = F)
...
emmip(m2.head.re3, ~ consistency2) #simple plotting

#consistency的主效应
emm_RT0 <- emmeans(m2.head.re3, ~ consistency2)
emm_RT0
update(pairs(emm_RT0), by = NULL, adjust = "holm") 

#type简单主效应
emm_RT1 <- emmeans(m2.head.re3, ~ type | consistency2)
emm_RT1
update(pairs(emm_RT1), by = NULL, adjust = "holm") 

#consistency的简单主效应
emm_RT2 <- emmeans(m2.head.re3, ~consistency2 | type)
emm_RT2
...
description1_df  <- data_df %>% group_by(consistency) %>%
  summarise(des_RT = mean(headAll), des_sd = sd(RT))

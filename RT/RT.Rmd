...
data_df$RT     <- data_df$RT * 1000 #将s的单位转换为ms
...
model_terms <- model.matrix(m1.full)  ### Please, please check the model_terms very very carefully. #see also:  https://rpubs.com/Reinhold/22193 and https://rpubs.com/yjunechoe/correlationsLMEM
...
#保留解释变异最大的三个随机效应(后续发现只要截距)
m1.re1 <- lmer(RT ~ 1  + type * consistency2 +
                 (1 | subid), 
                 data = data_df,
                 control = lmerControl(optimizer = 'bobyqa',
                                        calc.derivs = F,
...
summary(m1.re1)
isSingular(m1.re1)
summary(rePCA(m1.re1))
save(m1.re1, file = "m1_1res_RT.RData")
...
RT.sig1 <- anova(m1.re1,type = 3, test = 'F', ddf='Satterthwaite')  # F test
...
#这里我们使用LRT比较的结果，这会让我们的paper看起来更牛逼（
...
# also possible: 'satterthwaite', 'kenward-roger' for LMMs but not GLMMs

emmip(m1.re1, consistency2 ~ type) #simple plotting

#consistency的主效应
emm_RT0 <- emmeans(m1.re1, ~ consistency2)
emm_RT0
update(pairs(emm_RT0), by = NULL, adjust = "holm") 

#type的简单主效应
emm_RT <- emmeans(m1.re1, ~type | consistency2)
emm_RT
update(pairs(emm_RT), by = NULL, adjust = "holm") 
# using the FDR corrections here

#consistency的简单主效应
emm_RT2 <- emmeans(m1.re1, ~consistency2 | type)
emm_RT2

...
raw_data  <- read.csv(file="data_ap_original.csv",head=TRUE) #read data
data_df <- raw_data
...
#Retention of the one random effect that explain the most variation
m1.re1 <- lmer(RT ~ 1  + stage * consistency +
                 (1 | participant), 
                 data = data_df,
                 control = lmerControl(optimizer = 'bobyqa',
                                        calc.derivs = F,
...
save(m1.re1, file = "ap_m1.re1.RData")
...
emm_RT2 <- emmeans(m1.re1, ~consistency | stage)
emm_RT2
...
mean_df$consistency <- factor(mean_df$consistency)  #variables to factors
...
ggsave(p4, filename = "ap_plot.pdf", dpi = 600,
...
ggsave(s1, filename = "ap_time_course.pdf", dpi = 600,
